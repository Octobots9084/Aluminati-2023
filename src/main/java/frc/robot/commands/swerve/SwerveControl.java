...
public class SwerveControl extends CommandBase {
    ...
    public void execute() {
        ...
        xSpeed = MathUtil.fitDeadband(-leftJoystick.getY(), 0.04) * DriveTrain.MAX_SPEED*.15;
        ySpeed = MathUtil.fitDeadband(-leftJoystick.getX(), 0.04) * DriveTrain.MAX_SPEED*.15;

        // Calculate the deadband
        rot = MathUtil.fitDeadband(-rightJoystick.getX(), 0.04) * DriveTrain.MAX_ANGULAR_SPEED*.3;
